## v0.3.1
- Приложение теперь замечает вышедшее обновление само: проверка выполняется в фоне вскоре после запуска, затем периодически и при возврате в окно. Раньше уведомление могло появиться только после перезапуска приложения.
- О доступном обновлении сообщают системное уведомление и пункт «New update» в меню трея.
- Напоминание об обновлении можно скрыть — для каждой версии отдельно.
- Для Linux (x64) добавлен пакет `amd64.deb`.
- Внутренние улучшения стабильности: корректное восстановление после временных сетевых сбоев и более надёжная публикация обновлений.
